...
#---------------------------------------------------------------
module "db" {
  count   = var.create_db_airflow ? 1 : 0
...
module "airflow_s3_bucket" {
  ...
  bucket = "${local.aws_region}-${local.account_id}-airflow-logs"
...
#---------------------------------------------------------------
# CodeCommit Secret Credentials
#---------------------------------------------------------------

resource "aws_iam_user" "git_credentials" {
  count = var.enable_airflow ? 1 : 0
...
resource "aws_iam_policy_attachment" "code_commit" {
  count      = var.enable_airflow ? 1 : 0
  name       = "code_commit"
  users      = [aws_iam_user.git_credentials[0].name]
...
resource "aws_iam_service_specific_credential" "git_credentials" {
  ...
  user_name    = aws_iam_user.git_credentials[0].name
}
...
metadata:
   ...
   annotations:
    ...
    meta.helm.sh/release-namespace: "airflow"
data:
  GIT_SYNC_USERNAME: ${base64encode(aws_iam_service_specific_credential.git_credentials[0].service_user_name)}
  GIT_SYNC_PASSWORD: ${base64encode(aws_iam_service_specific_credential.git_credentials[0].service_password)}
YAML
}