...
#---------------------------------------------------------------
# S3 log bucket for Spark data, jobs and logs
#---------------------------------------------------------------
#tfsec:ignore:*
module "spark_s3_bucket" {
  count   = var.enable_spark_tools_team_a ? 1 : 0
  source  = "terraform-aws-modules/s3-bucket/aws"
  version = "~> 3.15"

  bucket = "${local.aws_region}-${local.account_id}-spark-team-a"
...
resource "aws_s3_object" "spark-event-logs" {
  count        = var.enable_spark_tools_team_a ? 1 : 0
  bucket       = module.spark_s3_bucket[0].s3_bucket_id
